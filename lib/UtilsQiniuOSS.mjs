 ...
 * @typedef {QiniuHttpClientRawOptions & { timeout?: number }} QiniuHttpClientOptions
 */

const { QINIU_HTTP_CLIENT_TIMEOUT } = process.env;
if (QINIU_HTTP_CLIENT_TIMEOUT) {
  const oldHttpClient = qiniu.httpc.HttpClient;
  // @ts-ignore
  qiniu.httpc.HttpClient = function (
    /** @type {QiniuHttpClientOptions} */ options,
  ) {
    options.timeout = options.timeout || Number(QINIU_HTTP_CLIENT_TIMEOUT);
    // @ts-ignore
    return oldHttpClient(options);
  };
}
