...
- 如果 `yy-detect-terminal` 技能不可用，使用最小化本地回退规则：先确认可用
  shell，再确认命令存在性判断方式，最后记录首选 shell、备用 shell、不可用
  shell 和搜索命令选择。
- `.terminal.local.md` 只描述本机环境，不代表其他开发者环境；发现记录失效时应立即更新。

## 需要遵守的规则

- 执行指令前，先检查项目根目录下是否存在 `AGENTS.LOCAL.md` 文件；如果存在，
  读取其中的内容并**严格遵守**；该文件用于存放开发者个人偏好配置，已添加到
  `.gitignore` 避免误提交
